...
import com.wisdom.classroom.entity.WisdomRoleMenu;
...
public class RoleAdminController {
    ...
    @Resource
    private WisdomRoleMenuService wisdomRoleMenuService;
    ...
    public Map<String, Object> list(JqgridBean jqgridbean
        ...
        List<WisdomRole> roleList = roleService.selectByExample(troleExample);
        PageRusult<WisdomRole> pageRusult = new PageRusult<WisdomRole>(roleList);
    ...
    @RequiresPermissions(value = {"角色管理"})
    public Map<String, Object> addupdaterole(WisdomRole trole) {
        LinkedHashMap<String, Object> resultmap = new LinkedHashMap<String, Object>();
        try {
            if (trole.getId() == null) {//新建
                ...
                WisdomRole oldObject = roleService.selectByKey(trole.getId());
                if (oldObject == null) {
                    resultmap.put("state", "fail");
    ...
    @RequiresPermissions(value = {"角色管理"})
    public Map<String, Object> deleteuser(WisdomRole trole) {
        LinkedHashMap<String, Object> resultmap = new LinkedHashMap<String, Object>();
        try {
            if (trole.getId() != null && !trole.getId().equals(0)) {
                WisdomRole role = roleService.selectByKey(trole.getId());
                if (role == null) {
                    resultmap.put("state", "fail");
                    ...
                    Example trolemenuexample = new Example(WisdomRoleMenu.class);
                    trolemenuexample.or().andEqualTo("roleId", trole.getId());
                    wisdomRoleMenuService.deleteByExample(trolemenuexample);
    ...
    @RequiresPermissions(value = {"角色管理"})
    public Map<String, Object> selectRoleById(WisdomRole trole) {
        LinkedHashMap<String, Object> resultmap = new LinkedHashMap<String, Object>();
        try {
            if (trole.getId() != null && !trole.getId().equals(0)) {
    ...
    public String loadCheckMenuInfo(Integer parentId, Integer roleId) throws Exception {
        List<WisdomMenu> menuList = wisdomMenuService.selectMenusByRoleId(roleId);// 根据角色查询所有权限菜单信息
        //移除所有没有pid的menuid
        Iterator<WisdomMenu> it = menuList.iterator();
        while (it.hasNext()) {
            WisdomMenu tmenu = it.next();
    ...
    private JsonArray getCheckedMenuByParentId(Integer parentId, List<Integer> menuIdList) {
        Example tmenuExample = new Example(WisdomMenu.class);
        tmenuExample.or().andEqualTo("pId", parentId);
        List<WisdomMenu> menuList = wisdomMenuService.selectByExample(tmenuExample);
    ...
    public Map<String, Object> saveMenuSet(String menuIds, Integer roleId) throws Exception {
        ...
        if (StringUtils.isNotEmpty(menuIds)) {
            ...
            List<WisdomMenu> menuList = wisdomMenuService.selectMenusByRoleId(roleId);
            //移除所有没有pid的menuid
            Iterator<WisdomMenu> it = menuList.iterator();
            while (it.hasNext()) {
                WisdomMenu tmenu = it.next();
            ...
            for (int i = 0; i < idsStr.length; i++) { // 然后添加所有角色权限关联实体
                WisdomRoleMenu trolemenu = new WisdomRoleMenu();
                trolemenu.setRoleId(roleId);
                trolemenu.setMenuId(Integer.parseInt(idsStr[i]));
                wisdomRoleMenuService.saveNotNull(trolemenu);
            }
        } else {
            resultmap.put("state", "fail");
    ...
    public String loadColumnsPermissionTree(Integer roleId) throws Exception {
        List<WisdomMenu> menuList = tmenuService.selectMenusByRoleId(roleId);// 根据角色查询所有权限菜单信息
        //移除有pid的menuid
        Iterator<WisdomMenu> it = menuList.iterator();
        while (it.hasNext()) {
            WisdomMenu tmenu = it.next();
    ...
    public Map<String, Object> saveColumnsSet(String permisIds, Integer roleId) throws Exception {
        ...
        if (StringUtils.isNotEmpty(permisIds.trim())) {
            ...
            for (int i = 0; i < namesStr.length; i++) { // 然后添加所有角色权限关联实体
                ...
                List<WisdomMenu> tmenuList = wisdomMenuService.selectByExample(tmenuExample);
                if (tmenuList == null || tmenuList.size() == 0) {
                    //当前menu表还没有这个name的记录，则先新增menu表的记录，再新增中间表的几率
                    WisdomMenu tmenu = new WisdomMenu();
                    tmenu.setName(namesStr[i]);
                    wisdomMenuService.saveNotNull(tmenu);

                    Example tmenuExample2 = new Example(WisdomMenu.class);
                    tmenuExample2.or().andEqualTo("name", tmenu.getName()).andIsNull("pId");
                    List<WisdomMenu> tmenuList2 = wisdomMenuService.selectByExample(tmenuExample2);
